...
class AgentState:
    ...
    def __init__(self, location=Coordinates(0, 0), orientation=Orientation(), visited_locations=set(), breeze_locations=set(), stench_locations=set(), has_arrow=True, heard_scream=False, glitter=False, has_gold=False, is_alive=True):
    ...
    def __copy__(self):
        return AgentState(self.location, self.orientation,
                          self.visited_locations, self.breeze_locations, self.stench_locations,
                          self.has_arrow, self.heard_scream, self.glitter, self.has_gold,
                          self.is_alive)
    ...
    def q_transform(self):
        ...
        combined_arr = np.hstack(
            (location_arr, orientation_arr, stench_arr, breeze_arr, visit_arr, int(self.has_arrow), int(self.heard_scream), int(self.glitter), int(self.has_gold)))
        ...
        if self.terminated:
            ...
            if action == Action.forward:
                ...
                new_percept = Percept(False, False, self._is_glitter(),
                                      False, False, is_terminated, 9999 if success else -1)
                return (new_environment, new_percept)
                ...
                new_percept = Percept(self._is_stench(), self._is_breeze(), self._is_glitter(),
                                      False, wumpus_killed, False, -1 if had_arrow else -11)
                return (new_environment, new_percept)
