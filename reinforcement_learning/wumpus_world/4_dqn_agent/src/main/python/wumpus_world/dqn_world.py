...
def main():
    def run_episode(environment, agent, state, mov=0):
        ...
        environment.agent = environment.agent.apply_move_action(next_action, 4, 4)
        ...
        if (not (next_percept.is_terminated)) & (mov < 50):
            next_percept.reward += run_episode(next_environment, agent, next_state, mov)
    ...
    agent.eval()
    total_reward = run_episode(initial_environment, agent, initial_state)
