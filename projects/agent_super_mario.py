...
from tensorflow.keras.layers import Dense, Activation, Flatten, Convolution2D, Permute, Dropout, Input, MaxPooling2D, Activation, AlphaDropout
...
model.add(Convolution2D(128, (6, 6), strides=(2, 2),
                        kernel_initializer="lecun_normal"))
model.add(Activation('selu'))
...
policy = LinearAnnealedPolicy(EpsGreedyQPolicy(), attr='eps', value_max=1., value_min=.05, value_test=.025,
                              nb_steps=nb_steps)
dqn = DQNAgent(model=model, nb_actions=nb_actions, memory=memory, nb_steps_warmup=nb_steps//40, processor=processor, enable_double_dqn=True, enable_dueling_network=False,
               target_model_update=4, policy=policy, gamma=.95, train_interval=2, delta_clip=1.)
dqn.compile(Adam(lr=1e-4, epsilon=1e-08), metrics=['mae'])


callbacks = [ModelIntervalCheckpoint(
    weights_filename, interval=nb_steps//20)]
callbacks += [FileLogger(log_filename, interval=nb_steps//20)]
callbacks += [TensorBoard(log_dir=LOG_DIR)]

dqn.fit(env, action_repetition=1, nb_steps=nb_steps,
        visualize=False,  callbacks=callbacks, verbose=2)
